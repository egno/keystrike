...
class AdaptiveGenerator:
    ...
    def generate_word(self, alphabet: frozenset[str]) -> str:
        word = ""
        for _ in range(MAX_RETRIES):
            word = self._sample_word(alphabet)
            if MIN_WORD_LEN <= len(word) <= MAX_WORD_LEN:
                return word
        return word

    def generate_lesson(
        self, alphabet: frozenset[str], focus_char: str, word_count: int = DEFAULT_WORD_COUNT,
    ) -> str:
        words = [self.generate_word(alphabet) for _ in range(word_count)]
